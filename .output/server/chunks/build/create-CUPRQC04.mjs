import { _ as __nuxt_component_0 } from './ConfirmationModal-BjWJ83bT.mjs';
import { defineComponent, ref, reactive, computed, watch, mergeProps, unref, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderList, ssrInterpolate, ssrRenderAttr, ssrRenderClass, ssrIncludeBooleanAttr, ssrLooseContain, ssrRenderStyle, ssrRenderComponent } from 'vue/server-renderer';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div></div></div></div><div class="w-full"><div class="bg-white rounded-2xl border border-gray-300 shadow-sm overflow-hidden"><div class="p-5 border-b border-gray-200 bg-gray-50/50"><h3 class="text-base font-bold text-gray-900">Kategori Booking</h3><p class="text-xs text-gray-500 mt-0.5">Pilih jenis booking untuk keperluan akademik.</p></div><div class="p-6 space-y-6"><div class="flex items-start gap-3 p-4 bg-blue-50 rounded-xl border border-blue-200"><input id="academic" type="checkbox"${ssrIncludeBooleanAttr(Array.isArray(unref(bookingForm).isAcademic) ? ssrLooseContain(unref(bookingForm).isAcademic, null) : unref(bookingForm).isAcademic) ? " checked" : ""} class="mt-1 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"><div class="flex-1"><label for="academic" class="block text-sm font-bold text-gray-900 cursor-pointer"> Booking untuk Keperluan Akademik </label><p class="text-xs text-gray-600 mt-1"> Centang jika booking ini untuk kegiatan akademik. </p></div></div>`);
      if (unref(bookingForm).isAcademic) {
        _push(`<div class="space-y-4 pl-4 border-l-4 border-blue-500"><div class="space-y-1.5"><label class="block text-xs font-bold text-gray-700 uppercase tracking-wider">Nama Institusi <span class="text-red-500">*</span></label><input${ssrRenderAttr("value", unref(bookingForm).institution)} type="text" required placeholder="Fakultas / Jurusan / UKM / Organisasi..." class="${ssrRenderClass([
          "block w-full rounded-xl border pl-4 pr-4 py-3 text-sm font-medium text-gray-900 focus:ring-2 shadow-sm transition-all",
        ...
        _push(`</div><div class="space-y-1.5"><label class="block text-xs font-bold text-gray-700 uppercase tracking-wider">Upload Surat Pengantar (PDF)</label><div class="relative"><input type="file" accept="application/pdf" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-xl cursor-pointer bg-gray-50 focus:outline-none focus:border-blue-500 file:mr-4 file:py-3 file:px-4 file:rounded-l-xl file:border-0 file:text-sm file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 transition-all"></div><p class="text-xs text-gray-500 mt-1"> 💡 Format file harus PDF, maksimal 5MB. </p>`);
      ...
      _push(`<div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-4"><button type="button" class="flex items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-5 py-3 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition-all active:scale-95"> Batal </button><button type="submit"${ssrIncludeBooleanAttr(unref(uploadProgress) !== null || unref(checkingAvailability) || unref(submitting)) ? " disabled" : ""} class="flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-blue-700 hover:shadow-md transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">`);
      if (unref(uploadProgress) !== null || unref(checkingAvailability) || unref(submitting)) {
      ...
      _push(`<span>${ssrInterpolate(unref(checkingAvailability) ? "Memeriksa Ketersediaan..." : unref(uploadProgress) !== null || unref(submitting) ? "Menyimpan..." : "Buat Booking")}</span></button></div></form>`);
...
//# sourceMappingURL=create-CUPRQC04.mjs.map

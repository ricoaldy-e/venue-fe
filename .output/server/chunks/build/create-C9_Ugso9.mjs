...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<button type="submit" form="create-field-form"${ssrIncludeBooleanAttr(loading.value) ? " disabled" : ""} class="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-bold text-white shadow-sm [@media(hover:hover)]:hover:bg-blue-700 [@media(hover:hover)]:hover:shadow-md transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">`);
      if (loading.value) {
      ...
      _push(`<span>${ssrInterpolate(loading.value ? "Menyimpan..." : "Simpan Lapangan")}</span></button></div></header>`);
      if (errorMsg.value) {
...
//# sourceMappingURL=create-C9_Ugso9.mjs.map

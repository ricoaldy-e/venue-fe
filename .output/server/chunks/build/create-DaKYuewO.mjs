  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(bookingForm).renterType !== "UMUM") {
        ...
        _push(`</div><div class="space-y-1.5"><label class="block text-xs font-bold text-gray-700 uppercase tracking-wider">Upload Surat Pengantar (PDF)</label><div class="relative"><input type="file" accept="application/pdf" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-xl cursor-pointer bg-gray-50 focus:outline-none focus:border-blue-500 file:mr-4 file:py-3 file:px-4 file:rounded-l-xl file:border-0 file:text-sm file:font-bold file:bg-blue-600 file:text-white [@media(hover:hover)]:hover:file:bg-blue-700 transition-all"></div><p class="text-xs text-gray-500 mt-1"> 💡 Surat pengantar dari institusi/fakultas (Wajib untuk Akademik/Tendik). </p></div></div>`);
      } else {
        _push(`<!---->`);
      }
      ...
      _push(`<div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-4"><button type="button" class="flex items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white px-5 py-2.5 text-sm font-bold text-gray-700 shadow-sm transition-all active:scale-95 [@media(hover:hover)]:hover:bg-blue-50 [@media(hover:hover)]:hover:text-blue-700 [@media(hover:hover)]:hover:border-blue-300"> Batal </button><button type="submit"${ssrIncludeBooleanAttr(unref(checkingAvailability) || unref(submitting) || unref(isUploadingFiles)) ? " disabled" : ""} class="flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-bold text-white shadow-sm [@media(hover:hover)]:hover:bg-blue-700 [@media(hover:hover)]:hover:shadow-md transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">`);
...
//# sourceMappingURL=create-DaKYuewO.mjs.map

...
import { a as availableIconsWithId } from './validIconList-DL-8OQus.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<button type="submit" form="create-facility-form"${ssrIncludeBooleanAttr(loading.value) ? " disabled" : ""} class="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-6 py-2.5 text-sm font-bold text-white shadow-sm [@media(hover:hover)]:hover:bg-blue-700 [@media(hover:hover)]:hover:shadow-md transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">`);
      if (loading.value) {
      ...
      _push(`<span>${ssrInterpolate(loading.value ? "Menyimpan..." : "Simpan Fasilitas")}</span></button></div></header>`);
      if (errorMsg.value) {
      ...
      if (filteredIcons.value.length > 0) {
        ...
        ssrRenderList(filteredIcons.value, (icon) => {
          _push(`<label class="${ssrRenderClass([[form.value.icon === icon.value ? "border-blue-500 bg-blue-50/50 shadow-md ring-1 ring-blue-500/20" : "border-gray-200 bg-white [@media(hover:hover)]:hover:border-blue-400 [@media(hover:hover)]:hover:bg-blue-50/30 [@media(hover:hover)]:hover:shadow-sm"], "group relative flex flex-col items-center justify-center p-3 rounded-xl border cursor-pointer transition-all duration-200"])}"><input type="radio"${ssrIncludeBooleanAttr(ssrLooseEqual(form.value.icon, icon.value)) ? " checked" : ""}${ssrRenderAttr("value", icon.value)} class="sr-only">`);
...
//# sourceMappingURL=create-CGgJuTc_.mjs.map

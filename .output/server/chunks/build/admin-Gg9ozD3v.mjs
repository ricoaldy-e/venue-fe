...
import { d as useState, n as navigateTo, u as useAppOptions } from './server.mjs';
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--><li><button type="button" class="${ssrRenderClass([[
        isMenuLainnyaActive.value ? "bg-blue-600 text-white shadow-sm" : "text-blue-200 hover:bg-ds-blue-800 hover:text-white"
      ...
      ssrRenderList(menuLainnya, (item) => {
        ...
        _push(ssrRenderComponent(_component_NuxtLink, {
          ...
          class: ["group relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200", [
            isActive(item) ? "bg-blue-500 text-white shadow-sm" : "text-blue-200 hover:bg-ds-blue-800 hover:text-white"
          ]],
          "aria-current": isActive(item) ? "page" : void 0
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<span class="${ssrRenderClass([[isActive(item) ? "text-white" : "text-blue-300 group-hover:text-white"], "w-4 h-4 flex-shrink-0 transition-transform duration-200"])}"${_scopeId}>`);
              ...
              return [
                createVNode("span", {
                  class: ["w-4 h-4 flex-shrink-0 transition-transform duration-200", [isActive(item) ? "text-white" : "text-blue-300 group-hover:text-white"]]
                }, [
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></ol></nav><div class="lg:hidden flex items-center min-w-0"><span class="text-sm font-semibold text-gray-900 truncate">${ssrInterpolate(breadcrumbs.value[breadcrumbs.value.length - 1]?.name)}</span></div></div><div class="flex items-center gap-2 sm:gap-3"><button type="button"${ssrIncludeBooleanAttr(topbarLogoutLoading.value) ? " disabled" : ""} class="group flex items-center gap-2 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 text-gray-700 bg-gray-100 hover:bg-red-50 hover:text-red-600 hover:shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-wait disabled:hover:scale-100 border border-gray-200 hover:border-red-200">`);
      if (!topbarLogoutLoading.value) {
        _push(`<svg class="w-4 h-4 transition-transform duration-200 group-hover:scale-110" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M16 13V11H8V8L3 12L8 16V13H16ZM20 3H12V5H20V19H12V21H20C21.1 21 22 20.1 22 19V5C22 3.9 21.1 3 20 3Z"></path></svg>`);
      } else {
...
//# sourceMappingURL=admin-Gg9ozD3v.mjs.map

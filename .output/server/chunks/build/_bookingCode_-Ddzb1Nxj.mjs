...
import { _ as __nuxt_component_0$1 } from './ConfirmationModal-CeBamt8k.mjs';
import { defineComponent, computed, ref, withAsyncContext, mergeProps, unref, withCtx, createBlock, createVNode, openBlock, useSSRContext } from 'vue';
...
import { u as useAppOptions, a as useRoute, c as createError } from './server.mjs';
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: `/admin/bookings/${unref(route).params.id}`,
      ...
      if (unref(pending)) {
        ...
        if (unref(booking).renterType === "AKADEMIK") {
          _push(`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-blue-100 text-blue-700 border border-blue-200"> AKADEMIK </span>`);
        } else if (unref(booking).renterType === "TENDIK") {
          _push(`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-purple-100 text-purple-700 border border-purple-200"> TENDIK </span>`);
        } else {
          _push(`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-gray-100 text-gray-700 border border-gray-200"> UMUM </span>`);
        }
        _push(`</div></div>`);
        if (unref(booking).renterType !== "UMUM" && unref(booking).institution) {
        ...
        _push(`</div>`);
        if (unref(booking).sptjmUrl || unref(booking).renterType !== "UMUM" && unref(booking).suratUrl) {
          _push(`<div class="sm:col-span-2 pt-4 border-t border-gray-100"><div class="flex flex-wrap gap-4">`);
          if (unref(booking).sptjmUrl) {
            _push(`<div><p class="text-xs font-semibold text-gray-500 mb-2">Surat SPTJM</p><a${ssrRenderAttr("href", unref(booking).sptjmUrl)} target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 [@media(hover:hover)]:hover:bg-blue-100 border border-blue-200 text-blue-700 rounded-lg text-sm font-semibold transition-colors"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z"></path></svg> Lihat SPTJM (PDF) </a></div>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(booking).renterType !== "UMUM" && unref(booking).suratUrl) {
            _push(`<div><p class="text-xs font-semibold text-gray-500 mb-2">Surat Pengantar</p>`);
            if (isPdf(unref(booking).suratUrl)) {
              _push(`<a${ssrRenderAttr("href", unref(booking).suratUrl)} target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-red-50 [@media(hover:hover)]:hover:bg-red-100 border border-red-200 text-red-700 rounded-lg text-sm font-semibold transition-colors"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z"></path></svg> Lihat Surat (PDF) </a>`);
            } else {
              _push(`<a${ssrRenderAttr("href", unref(booking).suratUrl)} target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 [@media(hover:hover)]:hover:bg-blue-100 border border-blue-200 text-blue-700 rounded-lg text-sm font-semibold transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> Lihat Gambar </a>`);
            }
            _push(`</div>`);
          } else {
            _push(`<!---->`);
        ...
        if (unref(booking)?.paymentStatus !== "PAID") {
          _push(`<button${ssrIncludeBooleanAttr(unref(updating)) ? " disabled" : ""} class="w-full px-4 py-2.5 bg-green-600 [@media(hover:hover)]:hover:bg-green-700 disabled:bg-gray-400 text-white text-sm font-semibold rounded-xl transition-all shadow-sm">`);
          if (unref(updating)) {
        ...
        _push(`</div></div>`);
        if (unref(booking)?.status !== "CANCELLED") {
          _push(`<div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6"><h3 class="text-sm font-bold text-gray-900 uppercase tracking-wide mb-4">Aksi Booking</h3><div class="space-y-3"><button${ssrIncludeBooleanAttr(unref(updating)) ? " disabled" : ""} class="w-full px-4 py-2.5 bg-white [@media(hover:hover)]:hover:bg-red-50 border border-red-200 disabled:opacity-50 text-red-700 text-sm font-bold rounded-xl transition-all shadow-sm flex items-center justify-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Batalkan Booking </button></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<div class="bg-gray-50 rounded-2xl border border-gray-200 p-5"><div class="flex items-start gap-3"><svg class="w-5 h-5 text-blue-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><div><p class="text-xs font-bold text-gray-900 mb-1">Informasi</p><p class="text-xs text-gray-600 leading-relaxed"> Booking dibuat pada ${ssrInterpolate(formatDate(unref(booking)?.createdAt ?? ""))}. </p></div></div></div></div></div>`);
      } else {
        _push(`<!---->`);
      }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/bookings/[id]/[bookingCode].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_bookingCode_-Ddzb1Nxj.mjs.map
